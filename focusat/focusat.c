/********************************************************
	...
	The image is passed to this program as an array maximum size of 512x512	
...
*/
	 ...
	 short  	 wide=512,high=512,binning=1; /* dimensions of chip */
	 long   	 pixelwide,pixelhigh;
	 int    	 CCDImage[512][512]; /* Image array */
/*	 float           raw[512][544]; */
	 float           raw[512][512];
	 ...
	 char *argv[]; 
{
	 ...
	 if (argc<2){
	  printf("Command format is- ./focusat /data/rd070222/junk486.raw \n");
	  printf("Results are written to -/data/rd070222/junk486.dat \n");
	  return(EXIT_FAILURE);
	 }
/*	 Estimate the threshold of the image. */
	 ...
	 if ((fpr=fopen(filenme,"rb"))==NULL){
	 	...
	 	if(fread(raw,(size_t) sizeof(float),(size_t) 512*512, fpr)!=512*512) {
...
/*	 		    printf("x = y diff = %i %i %i %i \n", stX, enX,stY,enY);  debug */
		...
		for (jjj=0;jjj<=numall;jjj+=1){
			...
			pxlsum=pxlsum+1.0;
			averagecount = averagecount+total;
...
/*	 Test to see if the stars from a line */
	 } else { /* didn't find any stars in the image */
	 	printf("Did not find any stars!! \n");
	        return(EXIT_FAILURE);
	 }
...
/*	for (jj=StartY;jj<=EndY;jj+=Sample) {
	...
	for (jj=StartY;jj<=EndY;++jj) {
		for (ii=StartX;ii<=EndX;++ii) {
			if(CCDImage[ii][jj] >= *pThresh-2.5*StandardError && 
			   CCDImage[ii][jj] <= *pThresh+2.5*StandardError) {
				num=num+1.0;
				Sum=Sum+(double)CCDImage[ii][jj];
				SumSquare = SumSquare+(double)CCDImage[ii][jj]*(double)CCDImage[ii][jj];
			} else {
